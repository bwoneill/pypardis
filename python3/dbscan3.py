...
from partition3 import KDPartitioner
from aggregator3 import ClusterAggregator
...
def map_cluster_id(key_id, broadcast_dict):
    """
    ...
    """
    cluster_id = next(iter(key_id[1])).strip('*')
    cluster_dict = broadcast_dict.value
    if '-1' not in cluster_id and cluster_id in cluster_dict:
        return key_id[0], cluster_dict[key_id[1]]
    else:
        return key_id[0], -1


def inflate_events(key_v, broadcast_dict):
    """
    :type key: int
    ...
    :return:
    """
    return [(p, key_v) for p, box in broadcast_dict.value.items()
...
if __name__ == '__main__':
    ...
    for i, t in enumerate(temp):
        ...
        in_box = [box2.contains([a, b]) for a, b in zip(x, y)]
    ...
    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(111)
    for i, b in dbscan.bounding_boxes.items():
        ax.add_patch(
