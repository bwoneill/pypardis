...
from geometry3 import BoundingBox
...
def map_results(key_v, broadcast_dict):
    """
    :type key: int
    :param key:
    ...
    :return:
    """
    for p, box in broadcast_dict.value.items():
        if box.contains(key_v[1]):
            return (key_v[0], p), key_v[1]
...
class KDPartitioner(object):
    ...
    def _min_var_split(self, index):
        ...
        if self.persist is None:
            partition = self.data.filter(lambda v: box.contains(v[1]))
        else:
            partition = self.partitions[index]
        k = self.k
        moments = partition.aggregate(np.zeros((3, k)),
                                      lambda x, k_v: x + np.array(
                                          [np.ones(k), k_v[1],
                                           k_v[1] ** 2]), add)
        ...
        bounds = np.array(
            [means[axis] + (i - 3) * 0.3 * std_dev for i in range(7)])
        counts = partition.aggregate(np.zeros(7),
                                     lambda x, k_v:
                                     x + 2 * (k_v[1][axis] < bounds) - 1,
                                     add)
...
if __name__ == '__main__':
    ...
    for label, box in kdpart.bounding_boxes.items():
        ax.add_patch(
